...
abstract class TutorialImpl(
    ...
    private fun createOverLay(
        ...
        val backgroundView = LayoutInflater.from(rootActivity).inflate(R.layout.tutorial_overlay, container, false)
        backgroundView.circle_overlay.centerX = x
        backgroundView.circle_overlay.centerY = y
        backgroundView.circle_overlay.gradientCenterX = x + (rootView.measuredWidth * bgBiasHorVert.first)
        backgroundView.circle_overlay.gradientCenterY = y + (rootView.measuredWidth * bgBiasHorVert.second)
        ...
        // Text
        val textView = LayoutInflater.from(rootActivity).inflate(params.textLayoutId, container, false)
        ...
        textView.applyGlobalLayoutListener {
            it?.let {
                it.x = tvXY.first
                it.y = tvXY.second + (if (textOnTop) it.measuredHeight / 2 else -it.measuredHeight / 2)
            }
        }
    ...
    fun measureView(view: View, bias: Float) {
        val rectangle = Rect()
